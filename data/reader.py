...
def readKeytermlist(keyterm_dir, query_dict):
    ...
    sorted_keyterm_list = sorted(
        keyterms.iteritems(), key=operator.itemgetter(1), reverse=True)
...
def readRequestlist(request_dir, fileIDs):
    ...
    request_list = sorted(requests.iteritems(),
...
def readTopicWords(topic_dir):
    ...
    for i in range(128):
        ...
        if not os.path.isfile(topic_filepath):
            logging.debug(
                "topic file {} does not exist!".format(topic_filepath))
            continue
