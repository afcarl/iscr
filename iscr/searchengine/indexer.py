...
class Indexer(object):
	...
	def run_indexing(self, transcript_dir, query_file = None, out_dir = None):
		...
		if query_file is not None:
			print("[Indexer] Query file specified. Building lex with query_file and documents...")
			files_to_read = [ query_file ] + docpaths
		else:
			print("[Indexer] Query file not specified. Building lex with documents only...")
	...
	def _build_indices(self, documents, index_pickle = None):
		...
		_end_time = time.time()
		print("[Indexer] Indexing done in {} seconds.".format(_end_time-_start_time))
		print("[Indexer] Total Memory of indices: {}".format(convert_size(asizeof.asizeof(indices))))
